...
| image | string | yes | 検知に使う画像ファイルのパス。保存時はルールJSONからの相対パスを優先する |
...
xとyはマルチディスプレイ環境で負の値になることがあるため、負数を許可する。

## 透過PNGマスク

PNG画像に透過部分がある場合、alpha=0のピクセルは検知対象から除外する。

これにより、毎回文字が変わる完了ウィンドウなどで、固定部分だけを比較対象にできる。
...
全ピクセルが透明な画像は、比較対象がないため検知エラーとして扱う。

GUIの`Edit Mask`では、元画像を直接上書きせず、`*.masked.png`という別ファイルを作成してルールのimageを差し替える。
...
- imageはルールJSONファイルからの相対パスとして扱う。
- regionはマルチモニター環境を考慮し、負の座標を許可する。
